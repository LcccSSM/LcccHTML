    ...
    <div class="login-container" :model="ruleForm">
      <el-form :model="ruleForm" ref="ruleForm">
        ...
        <div style="margin-top: 15px;">
          <b>昵称： </b>
          <b id="name" style="margin-left: 100px;">{{resturantName}}</b>
        </div>
...
<script>
  ...
  export default {
    name: 'UserInfo',
    data() {
      var checkbpassword = (rule, value, callback) => {
        if (!value) {
    ...
    methods: {
      search: function() {
        ...
        console.log(Uform)
        var url = this.axios.urls.SYSTEM_UACONUT_GETBYNAME;
        this.axios.post(url, Uform).then(resp => {
          console.log(resp);
      ...
      doMerge: function() {
        ...
        var url = this.axios.urls.SYSTEM_BANK_RECHARGE;
        console.log(re);
        this.axios.post(url, re).then(resp => {
          if (1 == resp.data) {
            this.dialogFormVisible = false;
            this.$message({
              message: '充值成功！',
    ...
    created: function() {
      this.username = this.$store.state.resturantName;
      this.search();
    },
    computed: {
